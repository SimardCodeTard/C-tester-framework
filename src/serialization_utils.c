...
static serialization_result_t read_exact(uint16_t fd, uint8_t *buf, uint32_t size)
{
	ssize_t		read_result;
	...
	{
		read_result = read(fd, buf + total_bytes_read, size - total_bytes_read);
		if (read_result > 0)
